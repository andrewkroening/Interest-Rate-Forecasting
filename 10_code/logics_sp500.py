...
def rate_modeler(
    ...
    if save_data:
        ...
        rate_data.to_csv(
            f"../20_intermediate_files/sp500_rate_data_{lookback}_{lookforward}.csv"
        )
    ...
    if save_model == True:
        ...
        joblib.dump(
            rate_model,
            f"../20_intermediate_files/sp500_rate_model_{lookback}_{lookforward}.pkl",
        )
...
def rate_forecast(lb, lf, df, save=True):
    ...
    # load the model pkl
    model = joblib.load(f"../20_intermediate_files/sp500_rate_model_{lb}_{lf-1}.pkl")
    ...
    for i in range(5218):
        ...
        # turn sample vals into a df with cols [3 Mo, 6 Mo, 1 Yr, 2 Yr, 3 Yr, 5 Yr, 7 Yr, 10 Yr, open, high, low, close]
    ...
    if save:
        ...
        df_forecast.to_csv(
            f"../20_intermediate_files/sp500_rate_forecast_{lb}_{lf-1}.csv", index=False
        )
...
def plot_forecast_pct(direc, df, fcast):
    ...
    # plot the original df
    ax.plot(df["Date"], df["3 Mo"], linewidth=1, color="grey", alpha=0.5)
    ax.plot(df["Date"], df["6 Mo"], linewidth=1, color="grey", alpha=0.5)
    ax.plot(df["Date"], df["1 Yr"], linewidth=1, color="grey", alpha=0.5)
    ax.plot(df["Date"], df["2 Yr"], linewidth=1, color="grey", alpha=0.5)
    ax.plot(df["Date"], df["3 Yr"], linewidth=1, color="grey", alpha=0.5)
    ax.plot(df["Date"], df["5 Yr"], linewidth=1, color="grey", alpha=0.5)
    ax.plot(df["Date"], df["7 Yr"], linewidth=1, color="grey", alpha=0.5)
    ax.plot(df["Date"], df["10 Yr"], linewidth=1, color="grey", alpha=0.5)
